...
namespace USAID.MapCODLocationsPreviewer.Web.UI.Controllers.Geo
{
    public class ProxyController : Controller
    {
        private static String version = "1.1.0";
        private static string PROXY_REFERER = System.Configuration.ConfigurationManager.AppSettings["ApplicationDomain"] + "/" + System.Configuration.ConfigurationManager.AppSettings["ApplicationName"] + "/Proxy/Index";//"http://localhost/proxy/proxy.ashx";
        ...
        private static string proxyConfigFile = System.Web.HttpContext.Current.Server.MapPath("~/proxy.config");
        //
        // GET: /Proxy/
        ...
        public void Index()
        {
            ...
            try
            {
                ...
                {
                    ProxyConfig proxyConfig = ProxyConfig.LoadProxyConfig(proxyConfigFile);
                ...
                else
                {
                    ...
                    if (!hasClientToken)
                    {
                        ...
                        {
                            ProxyConfig config = ProxyConfig.LoadProxyConfig(proxyConfigFile);
                            if (serverUrl.ClientId == null)
                                serverUrl.ClientId = config.Username;
                            if (serverUrl.ClientSecret == null)
                                serverUrl.ClientSecret = config.Password;
                            serverUrl.OAuth2Endpoint = config.TokenUrl;
                            if (serverUrl.Url.Contains("maps.itos.uga"))
                            {
                                serverUrl.OAuth2Endpoint = "https://maps.itos.uga.edu/arcgis/tokens/?request=getToken";
                            }
                ...
                try
                {
                    ...
                    log(TraceLevel.Info, e.ToString());
                } catch (Exception e) { }
...
*/
        ...
        {
            ProxyConfig config = ProxyConfig.LoadProxyConfig(proxyConfigFile);
            StringBuilder requestUrlSB = new StringBuilder(uri);
            requestUrlSB.Append("&username="); requestUrlSB.Append(config.Username);
            requestUrlSB.Append("&password="); requestUrlSB.Append(config.Password);
            requestUrlSB.Append("&expiration=2700");

            System.Net.HttpWebRequest req = (System.Net.HttpWebRequest)System.Net.HttpWebRequest.Create(requestUrlSB.ToString());
        ...
        private string getNewTokenIfCredentialsAreSpecified(ServerUrl su, string reqUrl)
        {
            ...
            if (isUserLogin || isAppLogin)
            {
                ...
                if (isAppLogin)
                {
                    ...
                    //string uri = su.OAuth2Endpoint + "token?username=" + su.ClientId + "&password=" + su.ClientSecret + &f=json";
                    if (!tokensURLMatch(su))
                        matchTokensUrl(su);
                      
                   
                    StringBuilder requestUrlSB = new StringBuilder(su.OAuth2Endpoint);
                    requestUrlSB.Append("&clientid=ref.");
                    requestUrlSB.Append(System.Configuration.ConfigurationManager.AppSettings["ApplicationDomain"]);
                    requestUrlSB.Append("&username="); requestUrlSB.Append(su.ClientId);
                    requestUrlSB.Append("&password="); requestUrlSB.Append(su.ClientSecret);
                    requestUrlSB.Append("&expiration=2700");

                    //string tokenResponse = webResponseToString(doHTTPRequest(requestUrlSB.ToString(), "POST"));
        ...
        private bool tokensURLMatch(ServerUrl s)
        {
            bool doesMatch = false;
            string tokensurl = Regex.Split(s.OAuth2Endpoint, "^(([^:/?#]+):)?(//([^/?#]*))?([^?#]*)(\\?([^#]*))?(#(.*))?")[4];
            string reqsurl = Regex.Split(s.Url, "^(([^:/?#]+):)?(//([^/?#]*))?([^?#]*)(\\?([^#]*))?(#(.*))?")[4];
            if (tokensurl.Equals(reqsurl))
                doesMatch = true;
            return doesMatch;
        }

        private void matchTokensUrl(ServerUrl s)
        {
            //use the valid match only to set the token url to the same server as the request url for valid only
            //in this case the mapsdev should not be used for maps where we know there are production services.
            //this addresses the limitation for the standard proxy to allow multiple token urls
            string sURL = Regex.Split(s.OAuth2Endpoint, "^(([^:/?#]+):)?(//([^/?#]*))?([^?#]*)(\\?([^#]*))?(#(.*))?")[4];
            string reqsurl = Regex.Split(s.Url, "^(([^:/?#]+):)?(//([^/?#]*))?([^?#]*)(\\?([^#]*))?(#(.*))?")[4];
            if (sURL.Contains("mapsdev.itos.uga.edu"))  //only allow use of prod services for dev
                s.OAuth2Endpoint = Regex.Replace(s.OAuth2Endpoint, sURL, reqsurl);
        }
        ...
        private bool parseKeywords(string keywords, string uri)
        {
            var match = false;
            try
            {
                String[] arrKeys = keywords.Split(",".ToCharArray(), StringSplitOptions.RemoveEmptyEntries);
                foreach (var s1 in arrKeys)
                {
                    if (uri.Contains(s1.Trim())) { return true; }
                }
            }
            catch (Exception e) { return match; }
            return match;
        }
        ...
        /*
        private string ReceiveTokenFromService(string referer)
        {
            string token = "";
            ...
            try
            {
                ...
                int ipHostEntryIndex = ipHostEntry.AddressList.Count() - 1; //use last entry if not localhost (ipIdx = 4)
                StringBuilder requestUrlSB = new StringBuilder(config.TokenUrl);
                string ipAddress = ipHostEntry.AddressList[ipHostEntryIndex].ToString();

                requestUrlSB.Append("&clientid=ref.");
                requestUrlSB.Append(System.Configuration.ConfigurationManager.AppSettings["ApplicationDomain"]);
                //requestUrlSB.Append("localhost");
                requestUrlSB.Append("&username="); requestUrlSB.Append(config.Username);
                requestUrlSB.Append("&password="); requestUrlSB.Append(config.Password);
                requestUrlSB.Append("&expiration=2700");
        ...
        */
        ...
        public class ProxyConfig
        {
            ...
            /*
            public static ProxyConfig LoadProxyConfig(proxyConfigFile))
            {
                ProxyConfig config = HttpRuntime.Cache["proxyConfig"] as ProxyConfig;

                if (config == null)
                {
                    string fileName = GetFilename(System.Web.HttpContext.Current);
            ...
            public ServerUrl GetConfigServerUrl(string uri)
            {
                ...
                foreach (ServerUrl su in serverUrls)
                {
                    ...
                    ProxyConfig config = ProxyConfig.LoadProxyConfig(proxyConfigFile);
                    su.ClientId = config.Username;
                    su.ClientSecret = config.Password;
            ...
            public override void Write(string message)
            {
                ...
                if (config.LogFile != null)
                {
                    ...
                    if (!log.Contains("\\") || log.Contains(".\\"))
                    {
                        ...
                        string configDirectory = System.Web.HttpContext.Current.Server.MapPath("proxy.config"); //Cannot use System.Web.Hosting.HostingEnvironment.ApplicationPhysicalPath b/ config may be in a child directory
                    ...
                    lock (_lockobject)
                    {
                        using (StreamWriter sw = System.IO.File.AppendText(log))
                        {
                            sw.Write(message);
                        }
                    }
